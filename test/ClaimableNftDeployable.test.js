...
const ClaimableNftDeployable = artifacts.require("./ClaimableNftDeployable");
const secondsInDay = 86400;
const secondsIn30Days = secondsInDay * 30;

contract("ClaimableNftDeployable", ([owner, user1, user2, user3, user4]) => {
  describe("Owner & beneficiaries", async function () {
    beforeEach("Deploy Claimable Nft contract", async () => {
      const args = [1, "SILVER BADGE", "SILVER", "https://baseurl.com"];
      claimNftInstance = await ClaimableNftDeployable.new(...args, { from: owner });
    });
    ...
    // transfer nft to another user and check balance
    describe("Transfer Nft", async () => {
      it("should allow user to send their nft", async () => {
        const args = [user1, user2, user3, user4];
        await claimNftInstance.addBulkUser(args, { from: owner });
        ...
        await claimNftInstance.mintNFT(...tokenUri, { from: user4 });
        const getTokenUri = await claimNftInstance.getTokenUri(user4);
        const balanceOfUser4 = await claimNftInstance.balanceOf(user4);
        const tokenId = await claimNftInstance.getTokenUri(user4);
        assert(+balanceOfUser4.toString(16) === 1);
         await claimNftInstance.safeTransferFrom(
          user4,
          user3,
          +tokenId.toString(16),
          { from: user4 }
        );
        const updatedbalanceOfUser4 = await claimNftInstance.balanceOf(user4);
        const balanceOfUser3 = await claimNftInstance.balanceOf(user3);
        assert(+updatedbalanceOfUser4.toString(16) === 0);
        assert(+balanceOfUser3.toString(16) === 1);
        const tokenIdUser4 = await claimNftInstance.getTokenUri(user4);
        const tokenIdUser3 = await claimNftInstance.getTokenUri(user3);
        console.log("tokenIdUser3",tokenIdUser3.toString(16));
        assert(+tokenIdUser3.toString(16) === +tokenId.toString(16));
        assert(tokenIdUser4.toString(16) === "");
      });
    });
  });
});
